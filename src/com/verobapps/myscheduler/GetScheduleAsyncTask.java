...
public class GetScheduleAsyncTask extends AsyncTask<Void, Void, Void> {
	 ...
	 *            Document object
	 */
	private void setUpHeaderText(Document doc) {

		// Splits string to get first date
		// eg. string: "Current Week - 10/13/2013 to 10/19/2013"
		String[] weekSpan = doc.select("div[id=dayTitle2]").text().split("-")[1]
				.split("to");
	...
	private String getShifts(Document doc) {
		...
		if (!mySchedule.isEmpty()) {
			...
			for (int i = 0; i < mySchedule.size(); i++) {
				...
				if (Calendar.getInstance()
						.before(mySchedule.get(i).getMyDate())) {
	...
	protected Void doInBackground(Void... params) {
		try {
			...
			// Used for testing with dummy schedule
			  ...
			  /*Document testDoc = Jsoup.parse(Util.tempHtml2); 
			  getShifts(testDoc);
			  tryAgain = false;*/
			 

			// Make sure user is logged in
			if (cm.getCookie(MyWebViewActivity.SCHEDULE_LINK) != null) {
				// Get session cookies
				String[] cookie = cm.getCookie(MyWebViewActivity.SCHEDULE_LINK)
						.split(";");
				...
				Document doc = Jsoup.connect(MyWebViewActivity.SCHEDULE_LINK).timeout(Util.JSOUP_TIMEOUT)
						.cookies(cookies).get();
